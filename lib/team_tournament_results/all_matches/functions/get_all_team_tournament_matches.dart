...
Future<AllTeamTournamentMatches> getAllTeamTournamentMatches(
  ...
  Map<String, List<TeamTournamentResultTeam>> matches = {};
  ...
  if (response.statusCode == 200) {
    ...
    for (Element row in rows) {
      if (!row.classes.contains("headerrow")) {
        if (row.classes.contains("roundheader")) {
          if (header.isNotEmpty) {
            matches.putIfAbsent(header, () => teams);
            teams = [];
          }
          header = row.text.trim();
        } else {
  ...
  return AllTeamTournamentMatches(
    buttons: buttons,
    matches: matches,
  );
}
